...
async def run_bulk(bulk_llm, historico, campos_desc, campos, json_model):
    json_template = json.dumps({c['campo']: "" for c in campos}, ensure_ascii=False)
    bulk = await bulk_llm.ainvoke([
...
async def run_edit(edit_llm, instrucao, campos, json_model, historico_edicao):
    json_template = json.dumps({c['campo']: "" for c in campos}, ensure_ascii=False)
...
async def run_fallback(fallback_llm, campo, historico) -> str:
